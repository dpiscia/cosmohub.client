angular.module('catalog', [
		...
		function(event, toState, toParams, fromState, fromParams){ 
			        ...
			        if ((user_auth.authorize(toState.access) === 0) ) {
			        	...
			        	//$location.path('/login').search('url',url);
			        	 location.assign("#/login?url="+$location.url());
			        	}
 ...
 .controller('HeaderCtrl', ['$scope','breadcrumbs', '$location' ,function HeaderCtrl($scope, breadcrumbs, $location) {
    ...
    $scope.show_nav = 
    	function() {
    		return ( $location.url().indexOf("/login") !== 0) && ($location.url().indexOf("/register") !== 0) && ($location.url().indexOf("/forgot_password") !== 0) && ($location.url().indexOf("/validation") !== 0);
    	}
    $scope.show_bread = function() {
    	return ($scope.breadcrumbs.getAll().length > 33);}
    }])
 .controller('nav_bar_Ctrl', ['$scope','user_auth','$location','$state', function HeaderCtrl($scope, user_auth, $location, $state) {
	$scope.show_nav = function() {return ($location.url().indexOf("/login") !== 0) && ($location.url().indexOf("/forgot_password") !== 0) && ($location.url().indexOf("/register") !== 0) 
								&& ($location.url().indexOf("/validation") !== 0) && ($location.url().indexOf("/public") !== 0);}
    ...
    angular.module('catalog').factory('myHttpInterceptor', function ($q) {
    return {
