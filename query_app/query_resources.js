angular.module('query')
   .factory('run_query_resources', function($http,$q){
    ...
    // return query result
    var query = {}
    query.run = function(raw_query,api_key,user_id, rows){
	    ...
	    deferred.resolve([angular.fromJson(data.result), angular.fromJson(data.limited)]);
	  	})
	...
	query.batch_query= function(raw_query,api_key,user_id, format)
	{	var deferred = $q.defer();
	  	$http({
	    method: 'POST',
	    url: '/api_python/batch_query',
	    data: {'query' : raw_query,
	    		'format' : format},
	     headers : { apiKey: api_key, user_id: user_id }
	   }).success(function(data, status, headers, config) {
