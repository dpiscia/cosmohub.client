...
.factory('tree_dict_from_flatten', [ function(){
		return function tree_dict_from_flatten(root_job, family, scope){
        	function date_adjust(data){ if (data !== null) return (data.split(".")[0] + data.split(".")[1]); else return data;}
	        if (!family) family = [];
	        var structura = { "name" : "" , "status" : "" , "id" : "" , "qc" : "", "size" : new Date(Date.parse(root_job.ts_ended))-new Date(Date.parse(root_job.ts_started)) };
	        structura.name = root_job.task;
	        structura.status = root_job.status;
	        structura.id = root_job.id;
	        structura.qc = root_job.qc;
	        var subjobs = [];
