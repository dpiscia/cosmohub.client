...
angular.module('paudm.user_auth').factory('user_auth', [ '$rootScope', '$http','$q', '$location','$state', '$cookieStore', function($rootScope, $http, $q, $location,$state,$cookieStore){
	...
	if ($cookieStore.get('real_name') != undefined) user.real_name = $cookieStore.get('real_name') ;
	...
	user.login =  function(username, password, rememberme) {
				...
				console.log(rememberme);
		                ...
		                if (rememberme){
			                ...
			                $cookieStore.put('real_name', user.real_name);
  ...
  user.getname = function() {
    return user.real_name;
  	...
  	user.forgot_password =  function(email) {
				var deferred = $q.defer();
				
					
	 	           	$http.get('/api_python/forgot_password/'+email, {  })
	 	           	.success(function(data){
		                console.log("OK");
  	...
  	user.validation =  function(email,token) {
				var deferred = $q.defer();
				
					
	 	           	$http.get('/api_python/validation?email='+email+'&token='+token, {  })
	 	           	.success(function(data){
		                console.log("OK");
